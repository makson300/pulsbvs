...
import { toDelimitedText } from '../domain/reportExport';
...
export function ReportsView({ drones, batteries, fleetState }: { drones: DroneAsset[]; batteries: BatteryAsset[]; fleetState: FleetState }) {
  ...
  const report = `Операционная сводка Пульс БВС\nПериод: ${filters.from || 'с начала учёта'} — ${filters.to || 'сегодня'}\nАктив: ${selectedAsset}\n\nВнесено вручную:\nПолётов: ${summary.flights.length}\nНалёт: ${summary.flightMinutes} мин\nЧек-листов: ${summary.checklistRuns.length}\nЗаписей с незавершёнными чек-листами: ${summary.incompleteChecklistFlightCount}\n\nКонтроль:\nОткрытых задач: ${summary.openTaskCount}\nКалендарных регламентов: ${summary.schedules.length}\nОткрытых событий: ${summary.openIncidentCount}\nОткрытых критичных событий: ${summary.criticalOpenIncidentCount}\nДокументов на учёте: ${summary.documents.length}\n\nВажно: сведения о полётах и календарные регламенты введены вручную; они не заменяют технические журналы и не являются автоматическим допуском.`;
  const csv = toDelimitedText([
    ['Тип', 'Дата', 'Актив', 'Название', 'Статус', 'Примечание'],
    ...summary.flights.map((flight) => ['Ручной вылет', flight.flightDate, drones.find((drone) => drone.id === flight.droneId)?.name ?? flight.droneId, flight.purpose, `${flight.durationMin} мин`, flight.note]),
    ...summary.tasks.map((task: MaintenanceTask) => ['Задача', task.createdAt.slice(0, 10), task.assetId, task.title, task.status, task.note]),
    ...summary.schedules.map((schedule) => ['Календарный регламент', schedule.nextDueDate, schedule.assetId, schedule.title, schedule.intervalDays ? `каждые ${schedule.intervalDays} дн.` : 'без периода', schedule.note]),
    ...summary.incidents.map((incident: IncidentRecord) => ['Событие', incident.occurredOn, incident.assetId, incident.title, incident.status, incident.description]),
    ...summary.documents.map((document: DocumentRecord) => ['Документ', document.createdAt.slice(0, 10), document.assetId, document.title, document.expiresOn ?? 'без срока', document.reference]),
  ]);
  return <>
