...
import type { AssetKind, AssetPassport, BatteryAsset, DocumentRecord, DroneAsset, FleetReadiness, IncidentRecord, MaintenanceTask } from '../domain/fleet';
...
export function FleetView({ drones, onAddDrone, onSavePassport }: { drones: DroneAsset[]; onAddDrone: () => void; onSavePassport: (kind: AssetKind, id: string, passport: AssetPassport) => void }) {
  return <>
...
export function MaintenanceView({ drones, batteries, tasks, incidents, documents, readiness, onAddTask, onTaskStatus, onAddIncident, onIncidentStatus, onAddDocument }: { drones: DroneAsset[]; batteries: BatteryAsset[]; tasks: MaintenanceTask[]; incidents: IncidentRecord[]; documents: DocumentRecord[]; readiness: FleetReadiness; onAddTask: (input: Omit<MaintenanceTask, 'id' | 'createdAt' | 'completedAt' | 'status'>) => void; onTaskStatus: (id: string, status: MaintenanceTask['status']) => void; onAddIncident: (input: Omit<IncidentRecord, 'id' | 'createdAt' | 'resolvedAt' | 'status'>) => void; onIncidentStatus: (id: string, status: IncidentRecord['status']) => void; onAddDocument: (input: Omit<DocumentRecord, 'id' | 'createdAt'>) => void }) {
  ...
  const [incident, setIncident] = useState({ title: '', occurredOn: new Date().toISOString().slice(0, 10), asset: '', severity: 'warning' as IncidentRecord['severity'], description: '' });
  ...
  return <>
    ...
    <section className="operations-list-grid">
      <RecordList title="Задачи обслуживания" empty="Задач пока нет." items={tasks} icon={<Wrench size={17} />} render={(item) => <><strong>{item.title}</strong><p>{label(item.assetKind, item.assetId)} · {item.dueDate ? `срок ${item.dueDate}` : 'без срока'}{item.note ? ` · ${item.note}` : ''}</p><small>{item.status === 'completed' ? 'Выполнено' : item.status === 'in_progress' ? 'В работе' : 'Открыто'}</small>{item.status !== 'completed' && item.status !== 'cancelled' && <button className="text-action" onClick={() => onTaskStatus(item.id, 'completed')}><CheckCircle2 size={15} />Выполнить</button>}</>} />
      ...
      <RecordList title="Документы и сроки" empty="Документы пока не добавлены." items={documents} icon={<FileText size={17} />} render={(item) => <><strong>{item.title}</strong><p>{item.documentType} · {label(item.assetKind, item.assetId)}{item.reference ? ` · ${item.reference}` : ''}</p><small>{item.expiresOn ? `Действует до ${item.expiresOn}` : 'Срок не указан'}</small></>} />
    </section>
  </>;
}
