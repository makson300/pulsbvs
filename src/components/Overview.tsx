...
function FirstRealFileGuide({ openUpload, pendingCount }: { openUpload: () => void; pendingCount: number }) {
  const steps = [
    ...
    ['3', 'Выберите рабочую копию', 'CSV/TXT/KML дадут ограниченную проверку. Для DAT/ZIP/JSON демо добавит в очередь только запись с метаданными; оригинал останется у владельца.'],
  ];
...
function PilotReadiness() {
  const readinessItems = [
    ...
    { status: 'done', title: 'Сложные файлы отмечаются для проверки', text: 'Для DAT/ZIP/JSON сохраняется только запись очереди с метаданными: по ним пока не строятся выводы и история полёта.' },
