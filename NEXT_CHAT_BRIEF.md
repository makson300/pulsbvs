...
- Тесты аналитики и доменной модели через Vitest:
  - последний проверенный результат после повторного аудита операционного этапа: 32 теста проходят.
...
- «Цифровой журнал полётов» — отдельный раздел/услуга с предполётным и послеполётным чек-листами; в «Отчётах» есть скачиваемая текстовая оперативная сводка.
- Повторный аудит усилил доменные ограничения: неполный чек-лист не сохраняется, черновые ответы не переходят к другой записи, ручной налёт суммируется только из журнала, задача проходит через «В работе», сроки документов показывают актуальный статус, а операционные даты используют локальный календарь браузера.
