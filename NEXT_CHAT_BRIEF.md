...
- Тесты аналитики и доменной модели через Vitest:
  - последний проверенный результат: 19 тестов проходят.
- Проектные Harvi skills находятся в `.harvi/skills/**` и отслеживаются Git.

## 4. Проверенное состояние перед handoff

Последние функциональные проверки перед handoff после DJI log intake pass:
...
- После усиления первого реального файла smoke подтвердил карточку происхождения, загрузку ZIP в очередь и отображение заметки рядом с файлом без внутренних/dev-терминов.
