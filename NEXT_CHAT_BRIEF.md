...
- Операционный контур в `FleetState` хранит паспорта дронов/батарей, задачи обслуживания, события, документы, ручные записи полётов и чек-листы. При загрузке старого состояния недостающие коллекции безопасно становятся пустыми.
...
Базовый ручной учёт паспортов, задач, событий, документов и готовности уже реализован в frontend-демо. Следующая часть этапа требует сопоставимой подтверждённой телеметрии.
