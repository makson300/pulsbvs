...
```text
b30d483 Harden local demo storage handling
...
- В настройках можно скачать и восстановить версионируемую JSON-копию локального `FleetState`. Она заменяет текущие записи демо, включает ручной операционный контур и результаты проверок, но не содержит оригиналы журналов или аккаунт. Чужие, повреждённые и несовместимые по версии файлы отклоняются.
...
- Отдельный Playwright-сценарий открывает настройки и восстанавливает только совместимую JSON-копию демо; unit-тесты проверяют её метку формата, версию и отказ повреждённых/чужих файлов.
