# Пульс БВС — инструкция для продолжения в новом чате

Этот файл нужен, чтобы начать новый чат без потери направления разработки и не повторять уже закрытую работу.

## 1. Стартовый промпт для нового чата

Скопируй в новый чат:

```text
Ты продолжаешь разработку проекта «Пульс БВС» в VS Code / Harvi Code.

Рабочая папка:
c:/Users/Mvoro/OneDrive/Рабочий стол/KIMI CODE

Сначала обязательно прочитай:
- PROJECT_STATE.md
- NEXT_CHAT_BRIEF.md
- PRODUCT_ANALYSIS.md
- README.md
- package.json

Затем проверь фактическое состояние:
- git status --short --branch
- git log --oneline -8

Если планируешь менять код, после изменений запускай релевантные проверки:
- npm test
- npm run build
- npm audit --omit=optional

Отвечай всегда на русском языке.
Telegram не использовать.
Не выдумывай факты и не обещай аналитику, если данных недостаточно.
DAT/ZIP/JSON не считать поддержанными для реальной аналитики/истории до подтверждённого чтения формата и схемы.
Не вставляй токены/секреты в чат, код, конфиги или документацию.
«Пульс БВС» работает исключительно с гражданской авиацией. Не исследуй, не позиционируй и не реализуй военные, оборонные, боевые, оружейные или неотделимые от них сценарии; при сомнительном назначении остановись и проверь границу.
Закрывай цельные проверенные этапы/пункты, а не бесконечные микрошаги: inspect → change → validate → commit → push.

Главная цель: довести «Пульс БВС» от рабочего frontend-демо до полезного продукта контроля гражданского флота БВС, происхождения журналов и операционного учёта. Технические тренды и предиктивные выводы возможны только после достаточной подтверждённой телеметрии. Текущий парк: DJI Avata 2, DJI Avata 360 и DJI Mini 4 Pro; T40 отложен до появления его журналов и отдельной проверки гражданского агросценария.

Текущий следующий безопасный инкремент: получить разрешённый обезличенный журнал Avata 2, Avata 360 или Mini 4 Pro по `docs/ИНСТРУКЦИЯ_ПО_ПОЛУЧЕНИЮ_ЖУРНАЛОВ_DJI.md`, подтвердить схему источника и только затем планировать адаптер. API-контракты backend и ADR уже подготовлены в `docs/`. Не начинай backend-код, декодирование DAT/ZIP или подключение `BATTERY_DEVICE_ERROR` к warning-строкам без подтверждённой схемы журнала.
```

## 2. Текущее состояние проекта

Постоянная продуктовая граница: только гражданская авиация. Для будущих моделей, маркетинга и партнёров нужно подтверждать гражданское назначение, законный доступ к данным и отсутствие неотделимого запрещённого сценария.

Проект сохранён локально и на GitHub:

```text
https://github.com/makson300/pulsbvs
```

Текущая ветка:

```text
main -> origin/main
```

Последние подтверждённые коммиты:

```text
1ccc850 Extract dashboard components
9e79b9e Extract landing component
ce4b7a6 Add first real file intake flow
75b37a5 Simplify client-facing copy
735690b Track project Harvi skills
9e76312 Refactor dashboard app structure
bc4a4db Keep sidebar footer navigation visible
e9fa66e Add fleet assets and import history
3f7fb2f Restore sidebar footer navigation
9de86a2 Revise product roadmap from AirData analysis
47acd21 Handle unsupported DAT and ZIP imports
7ae79fb Harden telemetry parsing for malformed inputs
```

Текущий стек:

- Vite
- React
- TypeScript
- Vitest
- lucide-react
- localStorage для демо-сессии/демо-активов/истории импортов

Ключевые команды:

```bash
npm run dev -- --host 127.0.0.1
npm test
npm run build
npm audit --omit=optional
```

Dev server в момент memory-pass был активен как managed process `82`:

```text
npm run dev -- --host 127.0.0.1
http://127.0.0.1:5173/
```

Если новый чат не видит этот процесс, не считать это ошибкой проекта: просто проверить URL или запустить dev server заново при необходимости.

## 3. Что уже реализовано и не надо делать повторно

- Landing page в тёмно-сапфировом стиле.
- Демо-регистрация через `localStorage`.
- Dashboard с разделами:
  - Обзор;
  - Флот;
  - Батареи;
  - Полёты;
- Техническое обслуживание.
- Журнал полётов.
- Отчёты.
- Sidebar footer regression исправлена: `Главная`, `Настройки`, `Помощь` видимы снизу sidebar; есть `data-testid="sidebar-footer"`.
- `src/App.tsx` после Variant B стал компактным оркестратором состояния, обработчиков, текущего раздела и модального окна.
- Крупные UI-блоки вынесены по файлам: `src/components/Landing.tsx`, `DashboardShell.tsx`, `CommonCards.tsx`, `Overview.tsx`, `OperationsSections.tsx`, `JournalSection.tsx`, `FlightsSection.tsx`, `Modals.tsx`; общие типы и данные находятся в `src/appTypes.ts` и `src/appData.ts`.
- Демо-активы дронов и батарей вынесены в `src/domain/fleet.ts`.
- Для новой пустой установки активы соответствуют текущему парку: DJI Avata 2, DJI Avata 360 и DJI Mini 4 Pro. До подтверждённого журнала нет показателей здоровья, налёта и циклов; старый `localStorage` не сбрасывается и не мигрируется автоматически.
- В UI можно добавлять демо-дроны и демо-батареи; повторный ID батареи выбирает существующую запись вместо дубля.
- Импорт CSV/TXT как табличной телеметрии.
- Импорт KML как маршрутного источника.
- DAT/ZIP/JSON принимаются с честным статусом: чтение формата ещё не подтверждено, аналитика не запускается, полноценная запись истории не создаётся.
- Их содержимое не читается в память до появления подтверждённого пути разбора; неизвестные расширения не маскируются под CSV, а произвольный XML не определяется как KML.
- CSV/TXT/KML создают запись истории только при наличии распознанных точек; пустой файл остаётся в очереди проверки.
- Для загрузки действует лимит 10 МБ; карточка происхождения включает источник, дату/период, сценарий и скрытые данные.
- Data Quality Score.
- Паспорт импорта:
  - route-only;
  - basic battery;
  - extended battery.
- Первые батарейные алерты:
  - ограниченные данные;
  - разбаланс ячеек;
  - перегрев;
  - низкое напряжение только при явно заданном пороге подтверждённого профиля;
  - ускоренный разряд;
  - ошибка батареи как правило в коде, но не подключать к реальным warning-строкам без схемы.
- История поддерживаемых импортов в `localStorage`:
  - ключ `puls-bvs-fleet-state`;
  - имя файла и время;
  - выбранный дрон и батарея;
  - профиль данных, качество, алерты и исходный результат анализа;
  - открытие сохранённого анализа после перезагрузки;
  - есть `data-testid="import-history"`.
- Тесты аналитики и доменной модели через Vitest:
  - после расширения операционного контура: 36 тестов проходят.
- Операционный контур в `FleetState` хранит паспорта дронов/батарей, задачи обслуживания, ручные календарные регламенты, события, документы, ручные записи полётов и чек-листы. При загрузке старого состояния недостающие коллекции безопасно становятся пустыми.
- Паспорт, задача, событие, документ и ручной журнал — данные, введённые человеком и сохранённые в браузере. Они не заменяют исходный технический журнал, не создают телеметрию и не дают автоматический допуск к полёту.
- «Цифровой журнал полётов» — отдельный раздел/услуга с предполётным и послеполётным чек-листами. Вылет можно уточнить или удалить; при удалении исчезают лишь его чек-листы, а связанные события сохраняются без ссылки. Сохранённый чек-лист можно уточнить только как полный набор обязательных ответов.
- Задачи поддерживают ответственного, отмену и результат выполнения; событие можно связать с ручным вылетом и закрыть с пояснением; документы можно изменять или удалять.
- Календарный регламент создаётся вручную с датой следующего срока, необязательным периодом, активом, ответственным и заметкой. Он отображается как напоминание и входит в ручную выгрузку, но не создаёт задачу, не использует налёт/циклы/телеметрию и не меняет готовность без явной задачи.
- В «Отчётах» есть фильтры периода и актива, скачиваемые TXT- и CSV-сводки. Они строятся только по ручным операционным сведениям, не по телеметрии.
- Повторный аудит усилил доменные ограничения: неполный чек-лист не сохраняется, черновые ответы не переходят к другой записи, ручной налёт суммируется только из журнала, задача проходит через «В работе», сроки документов показывают актуальный статус, а операционные даты используют локальный календарь браузера. Незакрытые предупреждения и незавершённые чек-листы отражаются как операционное внимание, не как технический допуск.
- Проектные Harvi skills находятся в `.harvi/skills/**` и отслеживаются Git.

## 4. Проверенное состояние перед handoff

Последние функциональные проверки после audit-pass:

```text
npm test                 -> 26 passed
npm run build            -> ✓ built
npm audit --omit=optional -> found 0 vulnerabilities
git diff --check         -> clean
```

Browser/Puppeteer smoke ранее подтвердил:

- Stage 2 flow: импорт с выбранными `Agras T40 №02` + `BT-009` создаёт историю с `1 записей`.
- Сохранённый анализ открывается после навигации/перезагрузки.
- Добавление демо-дрона/батареи через UI работает.
- Sidebar footer видим: `Главная`, `Настройки`, `Помощь`.
- После refactor `App.tsx` чистый smoke подтвердил footer, историю импортов и связку активов.
- После подготовки к реальным логам upload modal показывает источники логов, ограничения DJI Fly / Assistant 2 / KML, чеклист готовности, кнопку шаблона `source.txt` и прокручивается на малых экранах.
- После полного Variant B component extraction browser smoke подтвердил landing, демо-кабинет, обзор, upload modal, селекторы дрона/батареи, очередь ZIP-файла без неподтверждённой аналитики, основные разделы, помощь и настройки.
- После audit-pass smoke подтвердил landing, открытие демо-кабинета, footer, upload modal с селекторами, безопасную постановку синтетического ZIP в очередь, отображение источника/сценария в карточке файла, раздел «Полёты» и окно помощи без внутренних терминов.

Memory-pass сам должен проверяться как docs-only change через `git diff --check`, `git status --short --branch`, commit и push.

## 5. Глобальная дорожная карта разработки

### Этап 1. Импорт и объяснимость данных — завершён

Реализованы паспорт импорта, список распознанных и отсутствующих полей, шаблоны CSV/KML/source.txt, защита от пустых и повреждённых данных, а также честная обработка DAT/ZIP/JSON без имитации анализа.

### Этап 2. Основание первого пилота: активы и история импортов — завершён в frontend-демо

Реализованы доменные типы `DroneAsset`, `BatteryAsset`, `SavedTelemetryImport`, `PendingTelemetryImport`, localStorage-хранилище `puls-bvs-fleet-state`, выбор дрона и батареи при импорте, карточка происхождения первого файла, добавление демо-дронов/батарей, история поддерживаемых импортов, очередь исследования неподдержанных файлов, открытие сохранённого анализа после перезагрузки, явное разделение синтетических примеров и пользовательских импортов, sidebar footer fix и полный вынос крупных dashboard-компонентов из `App.tsx`.

Ограничения этапа: это локальный frontend-контур без backend/БД; DAT/ZIP/JSON не создают полноценную запись истории, пока нет подтверждённого чтения формата. Синтетические примеры теперь не создают локальных записей и никогда не должны выдаваться за реальные данные. [API-контракт](docs/BACKEND_API_CONTRACT_V1.md) сверен с карточкой происхождения файла (`source_context`), [ADR](docs/adr/0001-node-postgresql-backend.md), [требования к T40-журналам](docs/DJI_AGRAS_T40_ANONYMIZED_LOG_REQUIREMENTS.md), [общая инструкция по журналам DJI](docs/ИНСТРУКЦИЯ_ПО_ПОЛУЧЕНИЮ_ЖУРНАЛОВ_DJI.md) и [инструкция по логам с телефона/пульта](docs/ИНСТРУКЦИЯ_ПО_ЛОГАМ_С_ТЕЛЕФОНА_И_ПУЛЬТА_DJI.md) готовы, но production-контур и адаптер отсутствуют.

### Этап 3. Достоверный адаптер первой подтверждённой DJI-модели

Цель: заменить универсальные предположения подтверждённой схемой реального журнала одной модели текущего парка.

1. Получить обезличенные реальные журналы Avata 2, Avata 360 или Mini 4 Pro и приложить заполненный `source.txt`: происхождение, версию ПО, путь выгрузки, единицы и доступные поля.
2. Написать отдельный адаптер подтверждённого формата и диагностику неизвестных/изменившихся колонок.
3. Ввести версионируемый профиль выбранной модели: обязательные поля, диапазоны, пороги и ссылки на источники правил.
4. Подключить ошибки батареи только по подтверждённым кодам/строкам журнала.
5. Добавить фикстуры реальных обезличенных логов и контрактные тесты.

Критерий готовности: каждый вывод показывает фактические входные данные, версию правила, confidence и ограничение данных.

### Этап 4. История батареи и обслуживание

Цель: перейти от разового triage к раннему выявлению ухудшений.

Базовый ручной учёт паспортов, задач, событий, документов, готовности и календарных регламентов уже реализован в frontend-демо. Календарный регламент — это только ручное напоминание, не автозадача и не допуск. Следующая часть этапа требует сопоставимой подтверждённой телеметрии.

1. Хранить полёты, циклы, напряжения, температуру, разбаланс, расход и нагрузочный контекст по батарее.
2. Рассчитывать личную норму только по достаточной выборке сопоставимых полётов; иначе честно показывать нехватку данных.
3. Добавить объяснимые тренды батареи и историю решений инженера.
4. Создавать задачи ТО из алертов, вести статусы, исполнителя и подтверждающие документы/заметки.
5. Ручной календарный минимум завершён: следующий срок, необязательный период, актив, ответственный и заметка. Расписания по налёту и циклам отложены до подтверждённых данных и не должны подменять рекомендации производителя.

### Этап 5. Операционная готовность и отчётность

Цель: дать инженеру и руководителю проверяемый инструмент для пилота.

Локальный журнал полётов с двумя чек-листами и текстовая сводка уже реализованы. Ручной журнал остаётся отдельным от технических файлов.

1. Карта маршрута и временная шкала только по доступной телеметрии.
2. Отчёты по полёту, батарее и ТО с фактическими данными, алертами и ограничениями.
3. Экспорт HTML/PDF/CSV, история действий и хранение оригинала журнала.
4. Уведомления: email и webhook, затем web push. Telegram не использовать.
5. Роли, аудит, политика хранения, резервное копирование и наблюдаемость.

### Этап 6. Масштабирование по подтверждённому спросу

Цель: не повторять AirData целиком, а расширять продукт по результатам пилота.

1. Мультиорганизационность, площадки/проекты и тарифные лимиты.
2. Документированный API и интеграции с BI/GIS/CMMS.
3. Проверить на пилотах роли, юридически значимое подписание чек-листов, серверные оригиналы документов и требования к срокам хранения.
4. Новые модели БВС — только для подтверждённых гражданских сценариев, с отдельными адаптерами, фикстурами и профилями правил.
5. Streaming, погодные модели и сложное планирование — только при измеримом спросе и отдельном техническом обосновании.

Подробный анализ AirData и основания этой последовательности: `PRODUCT_ANALYSIS.md`.

## 6. Правила разработки

1. Не делать большие переписывания без необходимости.
2. Закрывать цельные этапы/пункты с проверкой, не возвращаться к пользователю после каждого мелкого шага.
3. Не имитировать реальную диагностику при нехватке данных.
4. Любое изменение аналитики покрывать тестами.
5. Для UI/import изменений запускать browser smoke через Puppeteer, если dev server доступен.
6. После значимых изменений запускать:

```bash
npm test
npm run build
```

7. Перед push по возможности запускать:

```bash
npm audit --omit=optional
git diff --check
git status --short --branch
```

8. После успешного шага:

```bash
git add .
git commit -m "краткое описание"
   git push
   ```

9. Не выходить за границу исключительно гражданской авиации: сомнительный сценарий, заказчик или данные требуют остановки и отдельной проверки, а не расширения scope.

## 7. Безопасность токенов и MCP

GitHub MCP подключается через переменную окружения:

```text
GITHUB_PERSONAL_ACCESS_TOKEN
```

Нельзя вставлять токены в чат, код, документацию или JSON-конфиг.

Если токен случайно попал в чат — считать его скомпрометированным и отозвать.

MCP-конфиг Harvi Code:

```text
C:/Users/Mvoro/AppData/Roaming/Code/User/globalStorage/kristianshin.harvi-code/settings/mcp_settings.json
```

Подключены:

- filesystem-puls-bvs;
- puppeteer;
- sequential-thinking;
- github.

Не подключены пока:

- Brave Search MCP — нужен API-ключ;
- Postgres MCP — будет нужен после появления backend/БД.

`.harvi/skills/**` теперь отслеживается Git. Остальные `.harvi/*` остаются ignored. Если новый чат не видит все project skills сразу, проверить файлы в `.harvi/skills/` и при необходимости перезапустить/перечитать проектный контекст.

## 8. Project skills, которые стоит использовать

В новом чате доступны или должны быть доступны из `.harvi/skills/**`:

- `puls-bvs-adapter-designer` — проектирование адаптера DJI Agras T40 без неподтверждённых допущений.
- `puls-bvs-backend-contract-designer` — API contracts для активов, импортов, телеметрии, алертов и ТО.
- `puls-bvs-architecture-reviewer` — контроль разделения UI/domain/analytics/storage/API.
- `puls-bvs-browser-smoke-runner` — практический Puppeteer smoke после UI/import изменений.
- `puls-bvs-context-memory-keeper` — обновление памяти и инструкции перед переполнением контекста.
- `puls-bvs-docs-keeper` — синхронизация README/PROJECT_STATE/NEXT_CHAT_BRIEF/PRODUCT_ANALYSIS.
- `puls-bvs-frontend-guardian` — UI-регрессии, sidebar footer, upload modal, import history.
- `puls-bvs-import-pipeline-auditor` — аудит импорта от файла до истории/алертов/ограничений.
- `puls-bvs-product-analyst` — продуктовые ограничения и запрет обещаний без данных.
- `puls-bvs-real-log-researcher` — исследование реальных DJI Agras T40 логов.
- `puls-bvs-release-runner` — test/build/audit/browser smoke/diff/docs/commit/push.
- `puls-bvs-security-privacy-guardian` — секреты, privacy логов, token policy, localStorage.
- `puls-bvs-storage-guardian` — localStorage и будущий переход к backend persistence.
- Дополнительные skills в Git могут включать data integrity, roadmap, git safety, test architecture и UI regression.

## 9. Первый рекомендуемый шаг в новом чате

Не начинать adapter/backend наугад. До появления журналов автоматизировать browser smoke различия демо и реального импорта. Проверка CSV/TXT-выгрузки со специальными символами и ручные календарные регламенты уже реализованы и покрыты тестами. Затем переходить к достоверному пилоту по первой модели текущего парка.

**Минимальный полезный инкремент завершён: API contracts + backend ADR + инструкции по получению журналов текущего парка + frontend intake для реальных логов.**

Рекомендуемый порядок:

1. Добавить автоматизированный browser smoke: синтетический пример не меняет историю, реальный поддержанный файл появляется с выбранными активами, а DAT/ZIP/JSON остаётся в очереди.
2. Получить разрешённый обезличенный реальный журнал Avata 2, Avata 360 или Mini 4 Pro по [инструкции](docs/ИНСТРУКЦИЯ_ПО_ПОЛУЧЕНИЮ_ЖУРНАЛОВ_DJI.md) или [инструкции по логам с телефона/пульта](docs/ИНСТРУКЦИЯ_ПО_ЛОГАМ_С_ТЕЛЕФОНА_И_ПУЛЬТА_DJI.md), обязательно с заполненным `source.txt`.
3. Подтвердить происхождение, версии ПО, реальные имена полей, единицы и ограничения каждого источника.
4. Использовать [API-контракт](docs/BACKEND_API_CONTRACT_V1.md) и [ADR](docs/adr/0001-node-postgresql-backend.md) как границы будущей реализации, а `DroneAsset`, `BatteryAsset`, `SavedTelemetryImport`, `FleetState`, `createSavedImport`/`upsertImport` — только как прототип localStorage.
5. Не подключать backend-код, DAT/ZIP/JSON-декодер и `BATTERY_DEVICE_ERROR` по warning-строкам до подтверждённой схемы реального журнала.
6. После документов/кода выполнить релевантные проверки:

   ```bash
   npm test
   npm run build
   npm audit --omit=optional
   git diff --check
   git status --short --branch
   ```

7. Проверить diff, сделать commit и push.

## 10. Если новый чат начинается после этого memory-pass

1. Проверить, что последний commit/push с сообщением вроде `Update handoff memory for next chat` присутствует в `git log --oneline -5`.
2. Если commit есть — не переписывать memory docs снова без причины.
3. Если commit отсутствует — сначала завершить memory-pass: проверить docs diff, `git diff --check`, commit и push.
4. После этого продолжить с исследования разрешённого реального журнала выбранной модели в новом отдельном цикле, чтобы не потерять контекст.
